    ...
    try:
        if BOTLOG:
            Config.CATUBLOGO = await l313l.tgbot.send_file(
                BOTLOG_CHATID,
                "https://graph.org/file/acd366144fb702ae2bd25.jpg",
                caption="**᯽︙ بــوت ماتركس يـعـمـل بـنـجـاح ✓ **\n**᯽︙ ارسل `.الاوامر` لرؤية اوامر السورس**",
                buttons=[(Button.url("سورس ماتركس", "https://t.me/xo_ot"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
...
async def add_bot_to_logger_group(chat_id):
    ...
    try:
        ...
        try:
            ...
            LOGS.error(str(e))
#by @xo_ot بس اشوفك خامطه للكود اهينك وافضحك 

joker = {"@xo_ot", "@po_ot", "@zoUka"}
async def saves():
   for lMl10l in joker:
        try:
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        descript = "- عزيزي المستخدم هذه هي مجموعه الاشعارات يرجى عدم حذفها  - @xo_ot"
        photobt = await l313l.upload_file(file="l313l/razan/resources/start/xo_ot.JPEG")
        _, groupid = await create_supergroup(
            "مجموعة أشعارات ماتركس ", l313l, Config.TG_BOT_USERNAME, descript, photobt
        )
